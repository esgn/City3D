...
def parse_args():
    ...
    parser.add_argument("--output_dir", "-o",
                        help="Output directory for fixed results", default="data/IGN/results_ground_face_fixed")
...
def main():
    ...
    for filename in tqdm(os.listdir(args.city3d_results_dir)):
        ...
        # A bold hypothesis : the ground face is always the last face in face list
        # TODO: Detect ground face using altitude
        ground_face = faces[-1]

        # Get candidate vertices that lies very close to ground face edges
        candidate_vertices, candidate_count = get_candidate_vertices(ground_face, vertices, float(args.delta_point_in_line))

        # print(json.dumps(candidate_vertices, indent=1))
        # print(candidate_count)

        # Deal with candidate vertices
        # Here two options are possible
        # First one : "break" edges to add candidates vertices with their original coordinates
        # Second one : add to projected vertex coordinates to edges
        # Regardless of the choosen option new vertices must be added

        new_ground_face, new_vertices_list, number_vertices_added = add_candidates_to_face(ground_face, vertices, candidate_vertices, False)

        if(number_vertices_added > 0):
            modified_count += 1

        new_faces[-1] = new_ground_face

        write_obj_file(os.path.join(args.output_dir, filename), new_vertices_list, normals, new_faces)
